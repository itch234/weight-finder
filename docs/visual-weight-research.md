...
以下の初回調査は、要因を採用した理由の記録です。「理論を実装した」ことと「人間の構図評価に対して精度を検証した」ことは異なります。

- 本アプリはLab色差・勾配・位置などを合成した独自モデルです。[Hübner & Fillinger (2016)](https://www.frontiersin.org/journals/psychology/articles/10.3389/fpsyg.2016.00335/full) のDCMそのものではなく、同論文から本アプリの倍率、許容差、点数の妥当性は導けません。[同著者らの2019年の研究](https://journals.sagepub.com/doi/full/10.1177/2041669519856040) も、バランスと好ましさの関係が画像の種類に依存すると報告しています。
- [Judd et al. (2009)](https://people.csail.mit.edu/torralba/publications/wherepeoplelook.pdf) は、色やエッジだけでは人物・文字への注視を取り逃がすことを示しています。肌色に倍率を掛ける処理は顔検出の代わりにはならず、「顔は常に最優先」とする保証もありません。
- 肌色ゲートの正確な実装は、肌色らしさの平均が0.35以下で全適用、0.35〜0.55で線形に減衰、0.55以上で無効化です。「35%以上で無効化」という初回の記述は不正確でした。
- Sobel勾配はエッジ・細部の手掛かりであり、レンズのピント精度を測定するものではありません。三分割の点数も交点への位置合わせの目安です。

今回、写真の三分割モードに主役の手動指定を追加しました。利用者が選んだ元画像上の位置を各比率に変換し、枠外なら採点せず、写真比較にも同じ指定を使います。透明画像は表示・解析・JPEG保存を白背景に統一し、静止画の自動再解析にはライブ用の前フレーム追従を適用しません。

合成画像の回帰テストは実装の整合性を検証するものです。実写真の撮影意図・人間の評価との一致率や、実機カメラでの撮影成功率は未測定です。現段階の位置付けは「構図の確認・練習を助けるアプリ」であり、自動で写真の優劣を決める審査器ではありません。
